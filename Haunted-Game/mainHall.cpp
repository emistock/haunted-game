...
*******************************************************************************/
mainHall::mainHall(Space* n, Space* s, Space* e, Space* w) : Space(n, s, e, w)
{
  spaceName = "Main Hall";
  numRows = 11;
  numCols = 7;
...
*******************************************************************************/
void mainHall::changePos(Player* p, int x, int y, char input)
{
  gameBoard[x][y] = ' ';
  ...
  if (under == '*')
  {
    ...
    if (p->getIsAlive())
    {
      displayImage("flashlight.txt");
      p->addItem("flashlight");
      p->setHasFlashlight(true);
      showMap(p->getHasFlashlight());
    }
  }
}
...
*******************************************************************************/
void mainHall::showHint()
{
  cout << "You should explore until you find the flashlight." << endl;
  cout << "If you are having trouble, check around the edges of the map." << endl;
  cout << "Be careful on item tiles, monsters are more likely to appear!" << endl;
  cout << "The kitchen is to the north, the bathroom is to the west." << endl;
}

/*******************************************************************************
Displays a map for the room if user has the item flashlight
*******************************************************************************/
void mainHall::showMap(bool flashlight)
{
...
void mainHall::setPlayerMarker(int x, int y)
{
  gameBoard[x][y] = 'O';
}
